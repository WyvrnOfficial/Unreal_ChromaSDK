...
#pragma region API declare prototypes
	...
	*/
	RZRESULT ChromaAnimationAPI::CreateEffect(RZDEVICEID deviceId, ChromaSDK::EFFECT_TYPE effect, int* colors, int size, FChromaSDKGuid* effectId)
	{
		if (!_sIsInitializedAPI)
		{
			return -1;
	...
	*/
	RZRESULT ChromaAnimationAPI::DeleteEffect(const FChromaSDKGuid& effectId)
	{
		if (!_sIsInitializedAPI)
		{
			return -1;
	...
	*/
	RZRESULT ChromaAnimationAPI::SetEffect(const FChromaSDKGuid& effectId)
	{
		if (!_sIsInitializedAPI)
		{
			return -1;
